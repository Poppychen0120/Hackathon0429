...
obstacles = [(2.0, 3.0), (3.0, 1.0)]  # obstacle positions (px, py)
...
def animate_simulation(x_trajectory, u_trajectory, model, num_agents=2, additional_lines_or_scatters=None, save_path=None):
    ...
    def update(i):
        ...
        # Plot the moving target
        if moving_target is not None:
            goal_pos = moving_target(i)
            ax.scatter(goal_pos[0], goal_pos[1], color='green', s=100, label='Goal', marker='*')
